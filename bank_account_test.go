...
func Open(balance int64) Account {
  accountVersion := os.Getenv("ACCT_VER")
  if accountVersion == "LOCK" {
    return OpenWithLock(balance)
  }
  return OpenWithRWLock(balance)
}

func OpenWithRWLock(balance int64) Account {
  if account := lock.Open(balance); account == nil {
    return nil
  } else {
    return account
  }
...
func OpenWithLock(balance int64) Account {
  if account := rw.Open(balance); account == nil {
    return nil
  } else {
    return account
  }
...
func TestMoreSeqCases(t *testing.T) {
  ...
  if account := Open(-10); account != nil {
...
// benchmark.
func BenchmarkAccountOperationsLock(b *testing.B) {
	if testing.Short() {
...
func BenchmarkAccountOperationsRWLock(b *testing.B) {
	if testing.Short() {
		b.Skip("skipping benchmark in short mode.")
	}
	a := OpenWithRWLock(0)
...
func BenchmarkAccountOperationsParallelLock(b *testing.B) {
	if testing.Short() {
		b.Skip("skipping benchmark in short mode.")
	}
	a := OpenWithLock(0)
...
func BenchmarkAccountOperationsParallelRWLock(b *testing.B) {
	if testing.Short() {
		b.Skip("skipping benchmark in short mode.")
	}
	a := OpenWithRWLock(0)
